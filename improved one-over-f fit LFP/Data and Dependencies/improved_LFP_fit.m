...
%the resulting variable will either be named "FX_fit_ts", "FX_fit_ts_new", "WT_fit_ts_new", or "WT_fit_ts", 
%a convention we used to help keep the variables organized when they were
%created. For this analysis, it doesn't matter what the name is, so we
%rename the loaded variable to "ts_data"
if exist('FX_fit_ts') 
    ts_data = FX_fit_ts;
elseif exist('FX_fit_ts_new')
    ts_data = FX_fit_ts_new;
elseif exist('WT_fit_ts')
    ts_data = WT_fit_ts;
else
