...
%Function to fit 1/f to LFP data. 
%   Aperiodic component has 3 terms: offset, slope (chai), and knee
%   output gives the absolute, aperiodic, and periodic spectra in log scale
%   with frequencies for x-axis
%   Output also gives center frequencies and powers of periodic peak 1a and
%   peak 1b
...
%   whether you want to find the parameters for the periodic peaks (0 = no,
%   1 = yes).
...
%find the maximum frequency below 6 Hz
    ...
    test_range = ind-5:ind+5;
      ...
      x = input(prompt);
      if x == 0 %if this was not a good fit
          if isempty(mid_min)
          figure(625)
          [peak_1_freqs_temp,pow_1] = ginput(1);
          scatter(peak_1_freqs_temp,pow_1)
          else %instead find the maximum before the largest local minimum
               [~,peak_1_est] = max(smoothed_data(1:testing.loc(mid_min(1))));
                ...
                x = input(prompt); %if still not good, enter manually
                    if x == 0 
                        figure(625)
                        [peak_1_freqs_temp,pow_1] = ginput(1);
                        scatter(peak_1_freqs_temp,pow_1)
    ...
    %find the maximum power above 6 Hz  
          ...
          if isempty(mid_min)
            ...
            peak_params = [peak_1_freqs_temp, freq_peak_2, pow_1, pow]; 
             ...
             if snip >= 0
                ...
                x = input(prompt);
                if x == 0 %if fit is still not good, enter manually
                    figure(625)
                    [freq_peak_2,pow] = ginput(1);
                    scatter(freq_peak_2,pow)
                    peak_params = [peak_1_freqs_temp, freq_peak_2, pow_1, pow]; 
