...
% 36-61). The code will automatically run with the sample data in the
...
%ask user if they would like to implement edge trimming and/or zero-padding
...
promptly = "Would you like to also see a time-frequency analysis of the data with no zero padding? (Requires Chronux toolbox) Yes = 1, No = 0.";
...
for a = 1:size(ts_data,1)
    ...
    freq_band = freq_band_temp(num_zeros+1:end-num_zeros);
    else
    ...
    if edge_trimming == 1
        trim_length = 2*fs; 
        freq_band = freq_band(trim_length:end-trim_length-1);
        sample_ts = ts_data(a,trim_length:end-trim_length-1);
    end

    %use hilbert transform and find the amplitude envelope
